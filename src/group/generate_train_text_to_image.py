...
"""
    ...
    tags = ["stable-diffusion", "stable-diffusion-diffusers",
            "text-to-image", "diffusers", "diffusers-training"]
...
def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch):
    ...
    for i in range(len(args.validation_prompts)):
        ...
        with autocast_ctx:
            image = pipeline(
                args.validation_prompts[i], num_inference_steps=20, generator=generator).images[0]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="~",
        help="The path prefix to use for the dataset and output directory. This is useful when running in a cloud environment.",
    )
    parser.add_argument(
        "--based_unet_path",
        type=str,
        default=None,
        help="Path to the base unet model.",)
...
def main():
    ...
    if args.based_unet_path is not None:
        ...
        print(
            f'use pretrained model name or path: {args.pretrained_model_name_or_path}')
        unet = UNet2DConditionModel.from_pretrained(
    ...
    def preprocess_train(examples):
        ...
        if "file" in examples:
            examples["file_name"] = [s.split('/')[-1] for s in examples['file']]
        return examples
    ...
    def collate_fn(examples):
        ...
        if "index" in examples[0]:
            indexs = torch.stack([torch.tensor(example["index"])
                             for example in examples])
        else:
            filenames = [example["file_name"] for example in examples]
            filename_no_suffix = [filename.split('.')[0] for filename in filenames]
            index_from_filename = [int(i) for i in filename_no_suffix]
            indexs = torch.tensor(index_from_filename)            

        return {"pixel_values": pixel_values, "input_ids": input_ids, 'indexs': indexs}
    ...
    for epoch in range(1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.input_perturbation:
                new_noise = noise + args.input_perturbation * \
            ...
            my_materials['samples'].append(latents.float())
            my_materials['noise_targets'].append(target.float())
            my_materials['preds'].append(model_pred.float())
            my_materials['indexs'].append(batch['indexs'])
    for i in range(1):
        print(my_materials['samples'][i].shape)
        print(my_materials['noise_targets'][i].shape)
        print(my_materials['preds'][i].shape)
        print(my_materials['indexs'][i].shape)
        print()
    # save materials
    import pickle
    pickle.dump(my_materials, open(os.path.join(
        args.output_dir, 'samples_t.pkl'), 'wb'))
    print(
        f'materials saved to {os.path.join(args.output_dir, "samples_t.pkl")}')
