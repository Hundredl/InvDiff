...
class IPR():
    ...
    def precision_and_recall(self, subject, subject_feats=None):
        '''
        ...
        if subject_feats is None:
            ...
            distances = compute_pairwise_distances(subject_feats)
            radii = distances2radii(distances, k=self.k)
            manifold_subject = Manifold(subject_feats, radii)
    ...
    def compute_manifold_ref(self, path, feats=None):
        self.manifold_ref = self.compute_manifold(path, feats=feats)
    ...
    def compute_manifold(self, input, feats=None):
        '''
