...
do
    python src/test.py \
        ...
        --model_name "split_fairness_32111123-noeiil-nodelta-lambda${LAMBDA}-delta${DELTA_RATIO}-${DELTA_TYPE}-deltaparam${DELTA_PARAM}-${SOFT_GROUPER}-${HARD_GROUPER_NUM}-${PATH_GROUPER}-bs${BATCH_SIZE}-sc${LEARNING_RATE}-ac${AC_STEPS}-lr${LR_SCHEDULER}-wu${WARMUP_STEPS}" \
