...
accelerate launch --mixed_precision="fp16" src/train_text_to_image.py \
  ...
  --output_dir="invariant/ckpts/fairness/models/split_fairness_32111123-eiil-delta-lambda${LAMBDA}-delta${DELTA_RATIO}-${DELTA_TYPE}-deltaparam${DELTA_PARAM}-${SOFT_GROUPER}-${HARD_GROUPER_NUM}-${PATH_GROUPER}-bs${BATCH_SIZE}-sc${LEARNING_RATE}-ac${AC_STEPS}-lr${LR_SCHEDULER}-wu${WARMUP_STEPS}" \
